...
def parse_args():
    ...
    parser = argparse.ArgumentParser(description="Sanity check: train on retain, test on forget")
...
def train_sanity(train_loader, val_loader, args):
    print("-" * 20 + "开始训练!" + "-" * 20)
...
def main():
    ...
    log_path = args.log_root / f"PreSanityCheck_{args.dataset}_{args.model}.log"
    ...
    if not subjects:
        raise ValueError("No subjects available for sanity check evaluation.")
    ...
    result_df.to_csv(csv_path / f"PreSanityCheck_{args.model}.csv", index=False)
