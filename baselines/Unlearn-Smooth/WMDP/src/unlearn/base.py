...
class SAMTrainer(BaseTrainer):
    ...
    def combine_and_update(self):
        ...
        for p, fg, rg in zip(param_list, fg_list, rg_list):
            ...
            final_grad = f_grad + self.gamma * r_grad
...
class RSTrainer(BaseTrainer):
    ...
    def training_step(self, model: nn.Module, inputs: dict) -> torch.Tensor:
        ...
        forget_loss = self.forget_step_randomized(model, inputs, n_samples=3)
        
        model.zero_grad()

        retain_loss = self.retain_step(model, inputs)

        self.combine_and_update()
        total_loss = forget_loss + self.gamma * retain_loss
        return total_loss.detach() / self.args.gradient_accumulation_steps

    def forget_step_randomized(self, model: nn.Module, inputs: dict, n_samples=3) -> torch.Tensor:
        param_list = self.sam_state["param_list"]

        sum_forget_grad = [torch.zeros_like(p, device=p.device) for p in param_list]
        sum_loss = 0.0
        ...
        for _ in range(n_samples):
            ...
            perturb_list = []
            for p in param_list:
                noise = torch.randn_like(p) * self.sam_rho
                perturb_list.append(noise)

            with torch.no_grad():
                for p, noise in zip(param_list, perturb_list):
                    p.add_(noise)
            ...
            for i, p in enumerate(param_list):
                if p.grad is not None:
                    sum_forget_grad[i].add_(p.grad.detach())
        ...
        for i in range(len(sum_forget_grad)):
            sum_forget_grad[i].div_(n_samples)

        self.sam_state["forget_grad"] = sum_forget_grad

        avg_loss = sum_loss / n_samples
        return torch.tensor(avg_loss, device=self.args.device)

    def retain_step(self, model: nn.Module, inputs: dict) -> torch.Tensor:
        loss = self._compute_retain_loss(model, inputs)
        self.accelerator.backward(loss)
    ...
    def _compute_forget_loss(self, model: nn.Module, inputs: dict) -> torch.Tensor:
        raise NotImplementedError

    def _compute_retain_loss(self, model: nn.Module, inputs: dict) -> torch.Tensor:
        raise NotImplementedError
